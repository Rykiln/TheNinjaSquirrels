	...
	execute(msgObject, args, client) {
		msgObject.guild.members.fetch(msgObject.mentions.users.first())
			.then(noshowMember => {
				...
				// const channelNoShow = process.env.TNS_CHANNEL_WARNINGS; // Hellfire Dominion Warnings Channel
				// const channelNoShow = process.env.TNS_CHANNEL_WARNINGS; // Oops I Pulled Warnings Warnings Channel
				// const channelNoShow = process.env.TNS_CHANNEL_WARNINGS; // The Ninja Squirrels Warnings Warnings Channel
				...
				fs.readFile(process.env.TNS_JSON_WARNINGS, function(err, data){
					...
					console.log(warns);
					fs.writeFile(process.env.TNS_JSON_WARNINGS, JSON.stringify (warns, null, 4), err => {
						if (err) throw err;
					});
				msgObject.delete();
